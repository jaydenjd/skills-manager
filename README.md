...
Skill Manager 会在本地启动一个只监听 `127.0.0.1` 的 HTTP API，默认从 `19010` 开始寻找可用端口；如果端口被占用，会自动尝试后续端口。你也可以在 **Settings → 本地 API** 里关闭 API 或自定义起始端口，并查看当前实际端口。

安装包内会包含 CLI，不需要写入 `/usr/local/bin` 或手动修改 PATH。agent 可以按固定路径直接调用。
...
如果用户本身已经把 CLI 加入 PATH，也可以直接调用：

```bash
skill-manager status
skill-manager scan --scope installed
skill-manager publish --agent Codex --skill tdd --message "补充 mock 测试规范"
...
CLI 会自动探测 `19010-19109`，也可以通过环境变量指定：
...
这个入口适合让 agent 负责编辑 skill 文件，再调用 Skill Manager 发布版本、安装、卸载、恢复和查看状态。

项目也内置了一个给 agent 使用的 skill：

```text
skills/skill-manager/SKILL.md
```

它会告诉 agent：可以直接编辑 skill 文件，但发布版本、安装、卸载、恢复和冲突处理应该交给 Skill Manager CLI/API 完成。
